...
HTMLHttpRequest.prototype.parseForm = function (form) {
    with(this) {
        var str = '',
            ...
            inputs = [(form[gE] ? form[gE]('input') : form.all ? form.all.tags('input') : []), (form[gE] ? form[gE]('select') : form.all ? form.all.tags('select') : []), (form[gE] ? form[gE]('textarea') : form.all ? form.all.tags('textarea') : [])];
        for (var i = 0; i < inputs.length; i++) for (j = 0; j < inputs[i].length; j++) if (inputs[i][j]) {
...
HTMLHttpRequest.prototype.xmlhttpSend = function (uri, formStr) {
    with(this) {
        ...
        if (formStr && xmlhttp.setRequestHeader) xmlhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        if (xmlhttp.overrideMimeType) xmlhttp.overrideMimeType((/\.txt/i).test(uri) ? 'text/plain' : 'text/xml');
...
HTMLHttpRequest.prototype.submit = function (formRef, evt) {
    with(this) {
        ...
        var method = formRef.getAttribute('method'),
            uri = formRef.getAttribute('action');
...
RemoteFileLoader.prototype.getThread = function (destId) {
    with(this) {
        ...
        threads[thr].callback = new Function('doc', 'text', 'uri', 'with(' + myName + '){copyContent(doc,text,"' + destId + '");if(onload)onload(doc,uri,"' + destId + '")}');
        return threads[thr]
    }
};
...
RemoteFileLoader.prototype.copyContent = function (docDOM, docText, destId) {
    var src = docDOM ? (docDOM.getElementsByTagName ? docDOM.getElementsByTagName('body')[0] : (docDOM.body ? docDOM.body : null)) : null;
    ...
    if (src && src.innerHTML) dest.innerHTML = src.innerHTML;
        ...
        for (var i = 0; i < src.childNodes.length; i++) dest.appendChild(document.importNode(src.childNodes.item(i), true))
    } else if (docText) {
